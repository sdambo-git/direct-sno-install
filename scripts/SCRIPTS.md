...
you on the multinode profile (see the README note on fresh ISO names — there
is no `--replace`), and every prompt has a safe default so `--yes` is
CI-friendly.
...
| 8 | `08_verify_cluster.py` | After install | Copies kubeconfig and `oc` to the jump host and runs `oc get nodes`. Downloads the 4.12.0 client from mirror.openshift.com if `oc` is not on PATH. |
...
| 8 | `08_verify_cluster.py` | Copies kubeconfig + `oc` to the jump host; `oc get nodes`. Downloads oc if missing. |
